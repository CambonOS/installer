...
if [[ $GPU = vmware ]]
then echo "echo 'trizen --noconfirm -Sy virtualbox-guest-utils || exit 1' | su $USER || exit 1" | ARCH && echo 'echo -e "yes\n" | rcvboxadd setup; /sbin/rcvboxadd quicksetup all' | ARCH || ERROR
