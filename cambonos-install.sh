...
DM () {
	...
	then
		...
		echo "trizen --noconfirm -Sy xorg-server lightdm lightdm-gtk-greeter && systemctl enable lightdm.service" | ARCH && DONE || ERROR
...
INSTALL () {
	...
	case $CONT in
		...
		69)
			...
			echo "systemctl enable zramd.service; systemctl enable xdg-user-dirs-update.service" | ARCH
