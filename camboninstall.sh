...
STOP () {
	...
	umount /mnt/boot; umount /mnt/home; umount /mnt; swapoff $SWAP; rm -rf /mnt; mkdir /mnt
	exit
}
...
(echo -e "NOCOLOR='\033[0m'\nRED='\033[1;31m'\nGREEN='\033[1;32m'\nSALIDA=/mnt$SALIDA\nDOMINIO=$DOMINIO\nNOMBRE=$NOMBRE\nGRUB=$GRUB\nDISCO=$DISCO\nGDM=$GDM\nPASS=$PASS" && echo '
	...
	echo -e "\n>>Instalando trizen\c"
	echo -e "cd /tmp && git clone https://aur.archlinux.org/trizen.git && cd trizen && makepkg -si && exit || exit 1" | su - sysop && DONE || ERROR
